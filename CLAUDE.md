...
- [x] **M7 Windows build** — electron-builder **NSIS one-click installer** (`-Setup.exe`; per-user, installs/updates in place, `createDesktopShortcut: always`, data in `%APPDATA%` preserved across updates/uninstall) + GitHub Actions release on tag
...
- [x] **M11 Search/filter + stock drill-down** — Sales & Purchase get a search box (voucher/invoice, party, **or item** via `items_text` `GROUP_CONCAT` in the list query) + From/To date filters; Stock rows are clickable → `inventoryRepo.lots()` shows the available FIFO lots with the source voucher (purchase vendor / production / opening) each came from
